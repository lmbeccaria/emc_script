  ...
  foreach my $entry (@$data) {
    ...
    foreach my $cat (@$categories) {
      ...
      if ( $entry->{$cat} > $highest[$index]{cat_amount} ){
          ...
          # print "$entry->{$cat} > $highest[$index]{cat_amount}\n";  
        
        $highest[$index]{cat_amount} = $entry->{$cat} ;
        $highest[$index]{customer} = $entry->{name};
        #print "$cat:$highest[$index]{cat_amount}:$highest[$index]{customer}\n";  
    }

    #print "HIGHEST: $highest[$index]{cat_name}:$highest[$index]{cat_amount}:$highest[$index]{customer}:--[index]: $index\n";   
    }  
  } 
